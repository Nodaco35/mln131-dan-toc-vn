...
import { 
  Sun, Moon, Search, Gamepad2, 
  RotateCcw, X, Award, Compass, Sparkles, BookOpen, 
  ...
  if (error) {
    return (
      <div className="w-full h-full bg-linear-to-br from-amber-50 to-orange-100 dark:from-slate-800 dark:to-slate-900 flex flex-col items-center justify-center text-primary dark:text-amber-500 text-xs p-4 text-center border-b border-primary/10">
        <Sparkles className="w-8 h-8 mb-2 opacity-50 animate-pulse" />
        <span className="font-medium font-serif italic text-sm">{fallbackText}</span>
      </div>
    );
  }
...
const EthnicList = () => {
  ...
  const generateNewQuizQuestion = () => {
    ...
    const validEthnics = data.filter(e => e.content && (e.content.trang_phuc || e.content.am_thuc || e.content.phong_tuc || e.content.chuyen_ke));
    if (validEthnics.length === 0) return;

    const randomEthnic = validEthnics[Math.floor(Math.random() * validEthnics.length)];
    ...
    const regexName = new RegExp(randomEthnic.name.replace("Dân tộc ", ""), "gi");
    ...
    const choices = [correctName, ...incorrectChoices].sort(() => 0.5 - Math.random());
  ...
  const filteredEthnics = data.filter(ethnic => {
    ...
    const matchesRegion = selectedRegion === 'Tất cả' || region === selectedRegion;
    const matchesSearch = ethnic.name.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-bg-site font-sans text-text-main transition-colors duration-300 relative overflow-hidden pb-20">
      ...
      <div className="absolute bottom-[20%] right-[-10%] w-[35vw] h-[35vw] rounded-full bg-secondary/15 blur-3xl pointer-events-none animate-pulse-slow" style={{ animationDelay: '4s' }}></div>
      ...
      <header className="sticky top-0 z-40 w-full bg-bg-site/85 backdrop-blur-md border-b border-primary/10 py-4 px-6 md:px-12 flex justify-between items-center transition-all shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-semibold font-cursive text-primary dark:text-amber-400">Bản sắc Việt Nam</p>
          </div>
        </div>
      ...
      {/* Hero Section */}
      <section className="max-w-7xl mx-auto px-6 md:px-12 pt-8 md:pt-16 pb-8 text-center relative z-10">
        <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full border border-primary/20 bg-linear-to-r from-primary/5 to-secondary/5 text-primary dark:text-amber-400 mb-6 animate-float shadow-xs">
          <Sparkles className="w-5 h-5 text-secondary dark:text-amber-400" />
          <span className="font-cursive text-2xl lowercase first-letter:uppercase font-bold">Khám phá 54 Dân tộc anh em</span>
        </div>

        <h1 className="font-rigid text-5xl md:text-7xl font-extrabold text-primary dark:text-amber-500 tracking-wide uppercase leading-tight mb-4">
          Sắc Mầu Văn Hóa <br/>
          <span className="text-transparent bg-clip-text bg-linear-to-r from-primary to-orange-500 dark:from-amber-400 dark:to-emerald-400 font-black">
            Dân Tộc Việt Nam
          </span>
        </h1>
        <p className="max-w-2xl mx-auto text-text-muted text-lg md:text-xl leading-relaxed mb-10 font-medium">
          Khám phá chiều sâu lịch sử, nét độc đáo trong trang phục, ẩm thực, phong tục và những câu chuyện truyền thuyết thiêng liêng của các dân tộc anh em trên mảnh đất hình chữ S.
        </p>

        {/* Thanh tìm kiếm & bộ lọc */}
        <div className="max-w-3xl mx-auto bg-card-bg border border-primary/10 p-4 rounded-2xl shadow-xl backdrop-blur-md mb-12">
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <div className="relative flex-grow">
              ...
              <input 
                ...
                className="w-full pl-11 pr-4 py-3.5 rounded-xl border border-primary/15 bg-bg-site/60 focus:outline-hidden focus:ring-2 focus:ring-primary/50 text-text-main placeholder-text-muted/60 transition-all text-base font-semibold"
              />
              {searchTerm && (
                <button 
            ...
            {/* Selector Vùng Miền */}
            <div className="flex items-center gap-1.5 md:w-60 overflow-x-auto whitespace-nowrap scrollbar-none py-1">
              <span className="text-xs font-bold text-primary dark:text-amber-500 uppercase tracking-wider pl-2 md:hidden">Vùng:</span>
              {['Tất cả', 'Tây Bắc', 'Đông Bắc', 'Trường Sơn - Tây Nguyên', 'Đồng bằng & Nam Bộ'].map((region) => (
                <button
                  key={region}
      ...
      {/* Main Grid */}
      <main className="max-w-7xl mx-auto px-6 md:px-12 pb-16 relative z-10">
        ...
        {filteredEthnics.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
            {filteredEthnics.map(ethnic => {
              ...
              let badgeColor = "bg-orange-100 text-orange-800 dark:bg-orange-950/50 dark:text-orange-300";
              if (region === 'Tây Bắc') badgeColor = "bg-sky-100 text-sky-800 dark:bg-sky-950/50 dark:text-sky-300";
              else if (region === 'Đông Bắc') badgeColor = "bg-teal-100 text-teal-800 dark:bg-teal-950/50 dark:text-teal-300";
              else if (region === 'Trường Sơn - Tây Nguyên') badgeColor = "bg-amber-100 text-amber-800 dark:bg-amber-950/50 dark:text-amber-300";
              else if (region === 'Đồng bằng & Nam Bộ') badgeColor = "bg-rose-100 text-rose-800 dark:bg-rose-950/50 dark:text-rose-300";
              ...
              return (
                ...
                >
                  ...
                  <span className={`absolute top-3 left-3 z-20 px-2.5 py-0.5 rounded-full text-xs font-bold tracking-wider uppercase shadow-xs ${badgeColor}`}>
                    {region}
                  </span>
                  ...
                  <div className="w-full aspect-[4/3] bg-linear-to-b from-transparent to-black/20 overflow-hidden relative">
                    ...
                    <div className="absolute inset-0 bg-linear-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
                      <span className="text-white text-xs font-semibold flex items-center gap-1">
                        Khám phá chi tiết <ChevronRight className="w-3.5 h-3.5" />
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-5 flex-grow flex flex-col justify-between border-t border-primary/5">
                    ...
                    <div className="mt-4 pt-3 border-t border-primary/5 flex items-center justify-between text-sm text-text-muted">
                      <span className="italic font-serif font-semibold">Di sản Việt Nam</span>
          ...
          <div className="text-center py-20 bg-card-bg rounded-2xl border border-primary/10 max-w-lg mx-auto shadow-md">
            ...
            <h3 className="text-lg font-bold">Không tìm thấy dân tộc phù hợp</h3>
            <p className="text-sm text-text-muted mt-2">Vui lòng thử từ khóa khác hoặc chuyển bộ lọc vùng miền về "Tất cả".</p>
            <button 
              onClick={() => { setSearchTerm(''); setSelectedRegion('Tất cả'); }}
          ...
          <div className="bg-bg-site max-w-lg w-full rounded-3xl border border-primary/20 shadow-2xl overflow-hidden relative animate-float">
            ...
            <div className="bg-linear-to-r from-primary to-orange-600 dark:from-amber-500 dark:to-orange-700 p-5 text-white flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-display font-bold text-lg leading-tight">Thử Thách Di Sản</h3>
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                {quizQuestion.choices.map((choice, i) => {
                  let buttonStyle = "border-primary/20 hover:bg-primary/5 text-text-main";
                  let Icon = null;
                  
                  if (quizAnswered) {
                    if (choice === quizQuestion.correctAnswer) {
                      buttonStyle = "bg-emerald-100 dark:bg-emerald-950/60 border-emerald-500 text-emerald-800 dark:text-emerald-300 font-bold scale-[1.02]";
                      Icon = CheckCircle2;
                    } else if (choice === quizSelectedAnswer) {
                      buttonStyle = "bg-rose-100 dark:bg-rose-950/60 border-rose-500 text-rose-800 dark:text-rose-300 font-bold";
                      Icon = X;
                    } else {
                      buttonStyle = "opacity-40 border-primary/10";
                    }
                  } else {
                    buttonStyle = "hover:border-primary/50 hover:bg-primary/5 active:scale-98";
                  }
                ...
                <div className="mt-4 pt-4 border-t border-primary/10 flex flex-col sm:flex-row justify-between items-center gap-3">
                  <div className="text-xs text-text-muted">
                    {quizSelectedAnswer === quizQuestion.correctAnswer ? (
                      <span className="text-emerald-600 dark:text-emerald-400 font-bold">🎉 Chính xác! Bạn rất am hiểu văn hóa!</span>
                    ) : (
                      <span>
                        Đáp án đúng là: <strong className="text-primary dark:text-amber-400">{quizQuestion.correctAnswer}</strong>
                      </span>
                    )}
                  </div>
